...
const OwnerDashboard = () => {
  ...
  const [formData, setFormData] = useState({
    ...
    booking_time: "14:00:00", // Updated starting hours to 14:00
    booking_time_to: "14:30:00", // Updated default end time
    confirmed: "YES",
  });
  ...
  // Modified grid time generation logic to strictly start at 14:00
  const timeSlots = Array.from(
    { length: (22 * 60 - 14 * 60) / 15 + 1 },
    (_, i) => {
      const totalMins = 14 * 60 + i * 15;
  ...
  const availableSlots = useMemo(() => {
    ...
    let durationMins = selectedSizeObj ? selectedSizeObj.mins : 30;

    // Apply color calculation context rule directly here if color style is matching
    if (formData.Style?.toLowerCase() === "colored") {
      durationMins += 30;
    }
  ...
  const editAvailableSlots = useMemo(() => {
    ...
    // Apply color calculation context rule for edit slots
    if (editFormData.Style?.toLowerCase() === "colored") {
      durationMins += 30;
    }
  ...
  const calculateEndTime = (
    ...
    if (!startTime) return "14:30:00"; // Adjusted fallback window
    const sizeObj = sizeOptions.find(
      (s) => s.value?.toLowerCase() === sizeValue?.toLowerCase()
    );

    let duration = sizeObj ? sizeObj.mins : existingDurationMins || 30;

    // If style is "Colored", append the 30-minute processing overhead
    if (styleValue?.toLowerCase() === "colored") {
      duration += 30;
    }
  ...
  const handleOpenEditModal = (appo) => {
    ...
    setEditFormData({
      ...
      booking_time: appo.booking_time || "14:00:00",
      booking_time_to: appo.booking_time_to || "14:30:00",
  ...
  const handleManualSubmit = async (e) => {
    ...
    try {
      ...
      setFormData({
        ...
        booking_time: "14:00:00", // Updated starting hours match
        booking_time_to: "14:30:00",
        confirmed: "YES",
      });
  ...
  return (
    <div className="min-h-screen bg-[#050505] text-white p-6 font-sans">
      ...
      <div className="max-w-7xl mx-auto flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-10">
        <div className="flex items-center gap-6">
          <div className="flex items-center gap-4">
            <img
              src="/logo_uts.png"
              alt="UTS Logo"
              className="w-10 h-10 object-contain rounded-full border border-white/10 p-0.5 bg-black"
            />
        ...
        {/* RIGHT CONTROLS WRAPPER (Calendar Picker + Logout Button) */}
        <div className="flex items-center gap-3 w-full sm:w-auto justify-end">
          <div className="flex items-center bg-[#111] p-2 rounded-2xl border border-white/10 gap-4">
            <button
              ...
              className="text-white/60 hover:text-[#fac700] transition-colors"
            >
              <ChevronLeft size={20} />
            </button>
            <input
          ...
          <button
            ...
            title="Log Out"
            className="flex items-center justify-center bg-[#111] border border-red-500/20 text-red-400 hover:bg-red-950/30 hover:border-red-500/50 p-3.5 rounded-2xl transition-all"
          >
            <LogOut size={18} />
          </button>
        </div>
